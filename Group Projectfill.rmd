...
Allied to the map above, the top 6 performing destination airports, when ranked by punctuality are:
...
In order to further analyze flights delays, the impacts of weather, general flight congestion and the variety of planes in use will be considered
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center',fig.width=7, fig.height=4,  out.width="50%"}
weather <- nycflights13::weather
...
The correlation plot shows a very strong relationship (0.98) between average delays and the number of flights, suggesting congestion can be a big part to play in delays. Some of the weather values are linked with delays: wind (-0.6), rain (0.61) and visibility (-0.58), so these factors do seem to play a part but the number of flights is the strongest contributor to delays. Interestingly it can be seen that temperature has little to no correlation with the number of flights (-0.07), suggesting the New York airports don't necessarily have a seasonality that can be guaged by how hot or cold it is outside.


## Congestion
As noted above, congestion seems to have an adverse affect on punctuality, so this will be explored further. Analysis will be done to calculate average delays per month to be be compared with the number of flights per month leaving New York.
...
As seen is the graphics above highest delays occur in the busiest months in terms of flights (June and July) with the trends matching for the rest of the year except November and December delays increase. Presumably with New York being a 'commuter city' the holiday season is to blame here (Thanksgiving and Christmas) with people going home. So although the number of flights leaving New York isn't an exceptionally high number, people traveling in these months all travel around the same few days, causing major congestion delays in clusters.



To uncover whether any time is made-up by pilots along journeys flights were categorized as short, medium or long haul and delays were analyzed to find disparaties between the distance groups. It can be seen below that yes, in fact long haul flights had better punctuality, both in terms of on time arrivals and also lower numbers being delayed. Suggesting the longer flight times enabled more opportunity to make up time in the air. Looking to the individual airports, Newark appears to have the worst departure and arrivals delay record.
